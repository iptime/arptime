...
{
	if(wps_mode == WPS_START){
		$('[sid=\"WPSBTNTEXT_CONTENT\"]').text(M_lang['S_WPSCANCEL_STRING']);
		$('[sid=\"WPSBTNIMG_CONTENT\"]').attr('src','images/wpscancel.png');
		$('[sid=\"WPSTEXT_CONTENT\"]').text(M_lang['S_WPSDESC_STRING']);
	}
	else if(wps_mode == WPS_CONFIGURED){
		$('[sid=\"WPSBTNTEXT_CONTENT\"]').text(M_lang['S_WPSCONNECT_STRING']);
		$('[sid=\"WPSBTNIMG_CONTENT\"]').attr('src','images/wpsconnect.png');
		$('[sid=\"WPSTEXT_CONTENT\"]').text(M_lang['S_WPSCONFIGURED_STRING']);
	}
	else{
		$('[sid=\"WPSBTNTEXT_CONTENT\"]').text(M_lang['S_WPSCONNECT_STRING']);
		$('[sid=\"WPSBTNIMG_CONTENT\"]').attr('src','images/wpsconnect.png');
		$('[sid=\"WPSTEXT_CONTENT\"]').text('');
	}
}
...
function wpssetup_part_submit(val)
{
	var localdata = [];
	var mode = get_category_val();
	$('#loading').popup('open');
...
{
	$('[sid=\"S_WPS_BTN\"]').unbind('click').click(function(){
		var val = WPS_START;
		if(wps_mode == WPS_START){val = WPS_STOP;}
		else if(wps_mode == WPS_STOP || wps_mode == WPS_CONFIGURED){val = WPS_START;}
		else {val = WPS_STOP;}
		wpssetup_part_submit(val);
	});
}
...
function wlrule_onoff_event_add()
{
	$('[sid=\"LISTDATA\"] select').unbind('change').each(function(){
		sliderButtonEvent({object : $(this), arguments : [$(this)], runFunc : function($myObj){
			var mode = get_category_val();
			var idx = $myObj.attr('sid').replace(/[^0-9]/g, '');
			var localdata = [];
			var tmp = null;
...
function pageview_select_update(mode)
{
	if(config_data.use_5g != '0'){
		...
		$('[sid="PAGE_MENU"]').append('<option value=\"5g\"'+((!mode||mode=='5g')?' selected':'')+'>5GHz</option>');
		$('[sid="PAGE_MENU"]').append('<option value=\"2g\"'+((mode&&mode=='2g')?' selected':'')+'>2.4GHz</option>');
...
function append_main_line(ruletype, ruleidx, run, unlocking)
{
	var HTMLStr = '';
	HTMLStr += ('<div sid=\"LISTDATA\" class=\"'+((ruleidx%2)?'lc_greenbox_div':'lc_whitebox_div')+'\">');

	HTMLStr += ('<a href=\"#right_panel\" id=\"'+ruletype+'_'+ruleidx+'\" data-icon=\"false\">');
	...
	HTMLStr += ('<label class=\"ui-hidden-accessible\" for=\"'+(ruletype + ruleidx)+'_run\"></label>');
	HTMLStr += ('<select sid=\"'+(ruletype.toUpperCase()+ruleidx)+'_RUN\" id=\"'+(ruletype + ruleidx)+'_run\" data-role=\"slider\">');
...
function update_bssid_line(idval, configObj, identifier)
{
	if(identifier && identifier == 'D'){
		...
		if(wps_mode == WPS_START){
			if(parseInt(status_data.remaintime) == 0 || parseInt(status_data.remaintime) >= 120){
				return;
			}
			$('#'+idval+' [sid=\"WLRULE_IMG\"]').css('display','');
			$('#'+idval+' [sid=\"WLRULE_TITLE\"]').text(M_lang['S_WPSCONNECTING1_STRING']+
			(120 - parseInt(status_data.remaintime))+M_lang['S_WPSCONNECTING2_STRING']);
		}
		else{
			$('#'+idval+' [sid=\"WLRULE_IMG\"]').css('display','none'); 
			if(status_data.stopbyschedule && status_data.stopbyschedule == '1'){
				$('#'+idval+' [sid=\"WLRULE_TITLE\"]')
					.text(M_lang['S_DEFAULT_WIRELESS']+ M_lang['S_STOPBYSCHEDULER_STRING']);
			}
			else if(status_data.stopbyswitch && status_data.stopbyswitch == '1'){
				$('#'+idval+' [sid=\"WLRULE_TITLE\"]')
					.text(M_lang['S_DEFAULT_WIRELESS']+ M_lang['S_STOPBYSWITCH_STRING']);
			}else{
	...
	if(configObj.ismain == '1'){
		$('#'+idval+' [sid=\"WLRULE_TITLE\"]').text(M_lang['S_DEFAULT_WIRELESS']);
		/*if(config_data.stopbyschedule && config_data.stopbyschedule == '1'){
			$('#'+idval+' [sid=\"WLRULE_TITLE\"]').text($('#'+idval+' [sid=\"WLRULE_TITLE\"]').text() + M_lang['S_STOPBYSCHEDULER_STRING']);
		}
		else if(config_data.stopbyswitch && config_data.stopbyswitch == '1'){
			$('#'+idval+' [sid=\"WLRULE_TITLE\"]').text($('#'+idval+' [sid=\"WLRULE_TITLE\"]').text() + M_lang['S_STOPBYSWITCH_STRING']);
		}*/
	}
	if(configObj.run == '0'){$('#'+idval+' [sid=\"WLRULE_VALUE\"]').text(M_lang['S_DISABLED_STRING']);}
	else{$('#'+idval+' [sid=\"WLRULE_VALUE\"]').text(configObj.ssid);}
	var $flipSwitch = $('[sid=\"LISTDATA\"] [sid=\"'+(get_ruletype(idval).toUpperCase() + get_ruleindex(idval))+'_RUN\"]').val(configObj.run).slider('refresh');
	//view_flip_switch( $flipSwitch );
	}
}
...
function get_extendsetup_data()
{
	...
	$.getJSON('/cgi/iux_get.cgi', { tmenu : window.tmenu,smenu : window.smenu, act : 'data', category : get_category_val(), mode : 'extendsetup'})
		...
		if(json_validate(data, '') == true){
			...
			$('[sid=\"C_EXTENDSETUP_COUNTRY\"]').val(config_data.extendsetup.country).selectmenu('refresh',true);
			$('[sid=\"C_EXTENDSETUP_MODE\"]').val(config_data.extendsetup.mode).selectmenu('refresh',true);
			$('[sid=\"C_EXTENDSETUP_CHANNELWIDTH\"]').val(config_data.extendsetup.channelwidth).selectmenu('refresh',true);
...
function init_rightpanel_after(ruletype)
{
	if(ruletype == 'wlrule'){
		...
		//$('[sid^=\"C_\"]').each(function(){
		//	if($(this).attr('type') == 'slider'){view_flip_switch( $(this) );}
		//});
	}
}
...
iux_update_local_func['wl2gmode'] = function(identifier)
{
	if(identifier == 'C'){
		$('[sid="LISTDATA"]').remove();
		...
		for(var j = 0; j < config_data.bsslist_2g.length; j++){
			if(!config_data.bsslist_2g[j].run)	continue;
			if(config_data.bsslist_2g[j].ismain == '1'){
				if(config_data.bsslist_2g[j].run == '0')locking_obj('S_WPS_BTN', 'disabled');
				else{					
					if(config_data.extendsetup.wpsmode && config_data.extendsetup.wpsmode == 'off') locking_obj('S_WPS_BTN', 'disabled');
					else unlocking_obj('S_WPS_BTN', 'disabled');
				}
			}
			append_main_line('wlrule',j,config_data.bsslist_2g[j].run,
				(config_data.bsslist_2g[j].personallist=='nouse' && config_data.bsslist_2g[j].useenterprise=='0'));
			update_bssid_line('wlrule_'+j, config_data.bsslist_2g[j]);
		}
		$('[sid=\"EXTENDSETUP_STRING\"]').text(get_category_text() + ' ' + M_lang['S_EXTENDSETUP_STRING']);
		$('[sid=\"WPSBTNTEXT_CONTENT\"]').text(M_lang['S_WPSCONNECT_STRING']);
		$('[sid=\"WPSTEXT_CONTENT\"]').text('');
	}
	else if(identifier == 'D'){
		if(status_data && status_data.wps){
			if(!status_data.remaintime)	return;
			if(status_data.wps.status == 'WPS_STOP'){wps_mode = WPS_STOP;}
			else if(status_data.wps.status == 'WPS_START' || status_data.wps.status == 'WPS_PROCESSING'){wps_mode = WPS_START;}
			else if(status_data.wps.status == 'WPS_CONFIGURED'){wps_mode = WPS_CONFIGURED;}
			else{wps_mode = WPS_STOP;}
			update_bssid_line('wlrule_0', null, identifier);
		}
	}
};

add_listener_local_func['wl2gmode'] = function(idval)
{
	wlrule_onoff_event_add();
	wps_button_event_add();
}

iux_update_local_func['wl5gmode'] = function(identifier)
{
	if(identifier == 'C'){
		$('[sid="LISTDATA"]').remove();
		...
		for(var j = 0; j < config_data.bsslist_5g.length; j++){
			if(!config_data.bsslist_5g[j].run)	continue;
			if(config_data.bsslist_5g[j].ismain == '1'){
				if(config_data.bsslist_5g[j].run == '0')locking_obj('S_WPS_BTN', 'disabled');
				else{
					if(config_data.extendsetup.wpsmode && config_data.extendsetup.wpsmode == 'off') locking_obj('S_WPS_BTN', 'disabled');
					else unlocking_obj('S_WPS_BTN', 'disabled');
				}
			...
			append_main_line('wlrule',j,config_data.bsslist_5g[j].run,
				(config_data.bsslist_5g[j].personallist=='nouse' && config_data.bsslist_5g[j].useenterprise=='0'));
			update_bssid_line('wlrule_'+j, config_data.bsslist_5g[j]);
		}
		$('[sid=\"EXTENDSETUP_STRING\"]').text(get_category_text() + ' ' + M_lang['S_EXTENDSETUP_STRING']);
		$('[sid=\"WPSBTNTEXT_CONTENT\"]').text(M_lang['S_WPSCONNECT_STRING']);
		$('[sid=\"WPSTEXT_CONTENT\"]').text('');
...
add_listener_local_func['wl5gmode'] = function(idval)
{
	wlrule_onoff_event_add();
	wps_button_event_add();
}
...
$(document).ready(function() {
	...
	iux_init(window.tmenu, window.smenu, {name : 'category', value : loc}, 60000, true);
	...
	pageview_select_update(loc); iux_update_local();
	get_status_local();
	$('[sid=\"C_WANINFO_MACLIST\"]').attr('sid', 'L_WIRELESS_SEARCHLIST');
	$('[sid=\"S_POPUP_MACSELECT_TITLE_TEXT\"]').attr('sid', 'L_WIRELESS_TITLE');*/
});
...
function iux_update_local(identifier)
{
	if(!identifier || identifier == 'D'){	//list update
		...
		if(mode){current_mode = mode; mode = 'wl' + mode + 'mode';}
		else{
			for(var articleName in config_data){
                	        if(config_data.hasOwnProperty(articleName) && articleName != ''){
					var tp = articleName.split('_');
					if(tp[0] == 'bsslist'){
						mode = 'wl' + tp[1] + 'mode';	current_mode = tp[1];
					}
				}
			}
		}
		if(mode){
			if(!identifier){
				iux_update_local_func[mode].call(this, 'C');	listener_add_local(mode, null);
	...
	$("#right_content").load(
		...
		function(responseTxt, statusTxt, xhr) 
		{
			if (statusTxt == "success") 
			{
				...
				if(_idvalue == 'wlrule_0' || _idvalue == 'extendsetup_0')
