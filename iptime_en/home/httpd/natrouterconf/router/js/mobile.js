...
{
        $('div div .lc_submit_button').click(function() {
...
iux_update_local_func['main'] = function(identifier)
{
	...
	$('[sid = "LISTDATA"]').on( "taphold", function(event) {
		...
		config_data.selectedIndex = $(this).attr('id').split('_')[1];
...
{
	var new_config_data = new Object();
	new_config_data.routinglist =  config_data.routinglist;
	new_config_data.selectedIndex = -1;
...
{
	if(identifier != 'C' || config_data.routinglist.length == 1)
		return;
	for(var i = 0; i < config_data.routinglist.length; i++){
		if(!config_data.routinglist[i].type)
			continue;
	...
	if(config_data.selectedIndex != -1)
		$('.routing_list_checkbox').eq(config_data.selectedIndex).prop('checked', true).checkboxradio("refresh");
};
