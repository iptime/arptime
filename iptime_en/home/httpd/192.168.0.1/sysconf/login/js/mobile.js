...
init_func['init'] = function()
{
	...
	if(logininfo.httpauth == '1')
	{
		...
		if( logininfo.usecaptcha  == "1" )
			$('[sid="LOGIN_AUTH_STATUS"]').text( M_lang[currenthttpauth]+' : '+logininfo.timeout + M_lang['S_LOGIN_TIMEOUTDESC'] + ' / ' + M_lang['S_LOGIN_USE_CAPTCHA']);
		else
...
submit_local_func['account'] = function()
{
	...
	if(!check_textfield_value('LOGIN_ACCOUNT',regExp_text) )
		return;
	if(!check_textfield_value('LOGIN_PASSWORD',regExp_passwd) )
		return;
	if(logininfo.validcaptcha == "1")
	{
...
init_func['auth'] = function()
{
	...
	httpauth == '1'? $('[sid="S_LOGIN_TIMEOUTDESC"]').css('opacity','') : $('[sid="S_LOGIN_TIMEOUTDESC"]').css('opacity','0.4'); 
	...
	*/
	...
	if( smtpauth == '1')
	{
		if(!check_textfield_value('C_LOGIN_SMTPACCOUNT',regExp_text) )
			return;
		if(!check_textfield_value('C_LOGIN_SMTPPASSWD',regExp_passwd) )
			return;
	}
	$("#loading").popup("open");
