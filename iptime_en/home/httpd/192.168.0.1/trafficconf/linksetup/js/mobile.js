...
function update_linkstat_title()
{
	...
	$("#lc_stat_div .lc_tablecolumn_div:first-child >div:first-child >div:first-child p").text(M_lang["S_TABLE2_COLUMN1"]);
	$("#lc_stat_div .lc_tablecolumn_div .lc_portdata_div:first-child >div:last-child p").each(function() {
...
function update_linkstat_row(index)
{
	...
	for(var i = 0; i < size; ++i)
	{
		$("#lc_stat_div >.lc_tablecolumn_div:nth-child(" + (i + 1) + ") >.lc_portdata_div:nth-child(" + (index + 2) + ") :last-child p").text(
...
function set_stat_background_width()
{
	if(!status2_data)
		return;
	var stat_size = get_stat_size(), port_size = get_port_size();
	var sum = 0;
	$('#lc_stat_div >div:gt(0):not(:last-child)').each(function()
	{
		sum += parseInt($(this).css('width'));
	});

	sum += parseInt($('#lc_stat_div >div').eq(1).css('padding-left')) + 2;
	$('#maincontent >.lc_table2_div >.lc_contents_div').css("width", sum + "px");
}
...
function get_linkstat_data(_tmenu, _smenu, _args, _retime, _use_local_status)
{
        ...
        .fail(function(jqxhr, textStatus, error) {
                console.log("get_linkstat_data : " + textStatus);
        });
}
