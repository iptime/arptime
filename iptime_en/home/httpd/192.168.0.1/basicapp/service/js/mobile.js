...
submit_local_func['ftp'] = function(localpostdata)
{
	...
	else
	{
		...
		for(var i=0; i<MAX_USER; i ++)
		{
			if(check_input_error("C_FTP_USERID"+i, regExp_text) || check_input_error("C_FTP_PASSWD"+i, regExp_passwd))
			{
				result2 = false; break;
			}		
		}
...
submit_local_func['afp'] = function(localpostdata)
{
		...
		if( check_input_error("C_AFP_NAME", regExp_text) )
			return false;
                for(var i=0; i<MAX_USER; i ++)
                {
                        if(check_input_error("C_AFP_USERID"+i, regExp_text) || check_input_error("C_AFP_PASSWD"+i, regExp_passwd))
				return false;
                }
        }
        return true;
...
submit_local_func['samba'] = function(localpostdata)
{
	...
	else
	{
		...
		for(var i=0; i<MAX_USER; i ++)
		{
			if( check_input_error("C_SAMBA_USERID"+i, regExp_text) || check_input_error("C_SAMBA_PASSWD"+i, regExp_passwd) )
				{result2 = false; break;}
		}
		if(result1 && result2) return true;
	}
...
submit_local_func['torrent'] = function(localpostdata)
{
	...
	{
		if( !check_input_error("C_TORRENT_USERID", regExp_text) && !check_input_error("C_TORRENT_PASSWD", regExp_passwd) &&
...
submit_local_func['rsync'] = function(localpostdata)
{
	...
	{
		if( !check_input_error("C_RSYNC_USERID", regExp_text) && !check_input_error("C_RSYNC_PASSWD", regExp_passwd) &&
...
submit_local_func['url'] = function(localpostdata)
{
	...
	{
		if( !check_input_error("C_URL_USERID", regExp_text) && !check_input_error("C_URL_PASSWD", regExp_passwd) &&
