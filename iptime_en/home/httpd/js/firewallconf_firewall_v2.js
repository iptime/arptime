...
function All_Enable_FWForm(doc)
{
	...
	//EnableObj_For_FW(F.formselect);	EnableObj_For_FW(F.rule_name);		EnableObj_For_FW(F.disable);
...
function All_Disable_FWForm(doc)
{
	...
	//DisableObj_For_FW(F.formselect);	DisableObj_For_FW(F.rule_name);	DisableObj_For_FW(F.disable);
	...
	//DisableObj_For_FW(F.mac_search);
	if(F2 && F2.macselect){
...
function All_Clear_FWForm(mode,frm_doc)
{
	...
	if(mode == 'internet')
	{
		...
		//doc.getElementById('mac_search').style.display = 'none';
...
function onChangeInternalType(frm_doc)
{
	...
	if(F.internal_type.value == 'ip'){
		...
		doc.getElementById('MACSEARCHBOXTD').style.display = 'none';
		//doc.getElementById('mac_search').style.display = '';
		//Clear_MacBox();
...
/*function onChangedWirelessMode(doc)
{
	var F = document.firewall_fm;
			...
			sport, eport, direction, protocol, policy, days, stime, etime, disabled, priority, wireless)
{
	...
	if(mode == 'internet'){
		...
		//tObj = document.getElementById(name+'_DELETE');
		//if(tObj){
...
function check_FW_IPRange(gw_addr, netmask, frm_name)
{
	...
	//for(var i = 1; (document.getElementById(frm_name+i)) ; i++){
	for(var i = 1; (obj = eval('document.firewall_fm.'+frm_name+i)) ; i++){
		//element_list[i-1] = eval('document.'+frm_name+i);
...
function onApplyBtnClicked(gw_addr, netmask, max_count)
{
	...
	if(F.formselect.value == 'internet')
	{
		...
		//F2.view_mode.value = parent.frames[3].document.getElementById('fw_select_menu').value;
	...
	else if(F.formselect.value == 'site')
	{
		...
		//if((F.sites.value).split(',').length > 5){
		//	alert(FIREWALL_SITE_COUNT_UNVALID);
		//	F.sites.focus();	F.sites.select();	return;
		//}
...
function moveRowDown(row_name)
{
	...
	var doc = ifr.document;
	var row = doc.getElementById(row_name);
	//var anchor = row.nextElementSibling;
...
function moveRowCustom(idx,frm_fm,ifrm_doc)
{
	...
	for(var i = 1; i < idx; i++){
		if(refer)
			//refer = refer.nextElementSibling;
